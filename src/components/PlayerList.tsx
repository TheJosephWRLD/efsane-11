...
export default function PlayerList({ side = 'right' }: PlayerListProps) {
  ...
  const executeRollAnimation = useCallback((action: () => void) => {
    ...
    const interval = setInterval(() => {
      const randomSquad = mockSquads[Math.floor(Math.random() * mockSquads.length)];
  ...
  const draftList = rolledSquad?.players.map(player => {
    ...
    const canBePlaced = emptySlotPositions.includes(player.position) || 
                       (player.secondary_position && emptySlotPositions.includes(player.secondary_position));
