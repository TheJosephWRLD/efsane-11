...
export default function DroppableSlot({ index, player, allowedPosition }: DroppableSlotProps) {
  ...
  const handleSlotClick = (e: React.MouseEvent) => {
    ...
    if (activePlayerToPlace) {
      const canActivePlayHere = activePlayerToPlace.position === allowedPosition || 
                                activePlayerToPlace.secondary_position === allowedPosition;
      ...
      if (sourceSlotIndex !== null && isSlotFull) {
        ...
        const canTargetPlayAtSource = player.position === sourceAllowedPos || 
                                      player.secondary_position === sourceAllowedPos;
  ...
  if (currentPlayerInHand) {
    const canActivePlayHere = currentPlayerInHand.position === allowedPosition || 
                              currentPlayerInHand.secondary_position === allowedPosition;
    if (canActivePlayHere) {
      if (sourceSlotIndex === null) {
        ...
        if (!isSlotFull) {
           ...
           const currentFormation = FORMATIONS.find(f => f.id === formationId);
           const sourceAllowedPos = currentFormation?.positions.find((p: PositionConfig) => p.index === sourceSlotIndex)?.allowedPosition;
           isValidTarget = player.position === sourceAllowedPos || player.secondary_position === sourceAllowedPos;
        }
      }
    }
  }
